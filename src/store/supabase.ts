...
const mockSongs: Song[] = [
  {
    ...
    audioUrl: "https://www.w3schools.com/html/horse.mp3", // Exemplo de áudio que funciona
    genre: "Rock",
    difficulty: "beginner",
...
export const songService = {
  ...
  async search(query: string): Promise<Song[]> {
    ...
    const { data, error } = await supabase!
      ...
      .select("*")
      .or(
        `title.ilike.%${query}%,artist.ilike.%${query}%,lyrics.ilike.%${query}%,chords.ilike.%${query}%`
      )
      .order("created_at", { ascending: false });
...
console.log(
  isOfflineMode
    ? "🔌 Modo Offline Ativo - usando dados locais"
